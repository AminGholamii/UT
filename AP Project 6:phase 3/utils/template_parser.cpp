...
string TemplateParser::getHtml(map<string, string> _context) {
    ...
    std::map<std::string, std::string> safeContext;
    for (auto &kv : _context) {
        safeContext[kv.first] = escapeString(kv.second);
    }

    TemplateUtils::writeMapToFile(outputFolder + "/" + mapFile, &safeContext);
