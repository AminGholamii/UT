...
string User::showInvitations(){
    ...
    for (auto invitation : invitations) {
        ostringstream oss;
        oss << invitation->getID() << ": " 
            << invitation->getTitle() << " - " 
            << "on " << dateToStr(invitation->getDate()) << " - " 
            << "at " << invitation->getStartTime() << " - " 
            << "to " << invitation->getEndTime() << "<br><br>";
...
string User::reportAll(shared_ptr<Date> from, shared_ptr<Date> to){
