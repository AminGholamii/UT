...
public:
    ...
    void getInvitation(shared_ptr<JoinEvent> new_event);
    void addJoinEvent(shared_ptr<JoinEvent> new_event);
    string showInvitations();
    shared_ptr<JoinEvent> confirmInvitation(int id);
    void rejectInvitation(int id);

    string reportAll(shared_ptr<Date> from, shared_ptr<Date> to);
...
private:
    ...
    vector<shared_ptr<JoinEvent> > join_events;
    vector<shared_ptr<JoinEvent> > invitations;
