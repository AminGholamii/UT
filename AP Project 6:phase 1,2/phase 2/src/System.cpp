...
void System::applyCommand(string main_command, string second_command){
    ...
    else if ( second_command == JOIN_EVENT ){
    ...
    else if ( second_command == CONFIRM )
        confirmInvitation();

    else if ( second_command == REJECT )
        rejectInvitation();
}
...
void System::signUp(){
    ...
    try {
        ...
        string re_format = "\\s+(\\S+)\\s+\"(\\S+)\"";  // changed here 
...
void System::addJoinEvent(){
    ...
    try {
        ...
        if ( params.find("guests") == params.end() || params.find("date") == params.end() 
            || params.find("start_time") == params.end() || params.find("end_time") == params.end() 
            || params.find("title") == params.end() )
            throw bad_request();

        string guests_ = params["guests"];
        if ( guests_ == "\"\"")
            throw bad_request();
        ...
        istringstream str_stream(guests_);
        int commas_count = count(guests_.begin(), guests_.end(), ',');
        string username;
        ...
        shared_ptr<Date> date = strToDate(params["date"]);
        int start_time = stoi(params["start_time"]);
        int end_time = stoi(params["end_time"]);
...
void System::confirmInvitation(){
    ...
    try {
        ...
        string word;
        str_stream >> word;
        if ( word != "invitation_id" )
            throw bad_request();

        int entered_id;
        str_stream >> entered_id;

        auto new_event = currently_in_->confirmInvitation(entered_id);
...
void System::rejectInvitation(){
    ...
    try {
        ...
        currently_in_->rejectInvitation(entered_id);
        everythingOK();
    }
...
void System::report(){
    ...
    try {
        ...
        if ( type.empty() )
            ...
            currently_in_->reportJoinEvents(from, to);
